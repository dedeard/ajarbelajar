...
<div class="mb-3 border bg-white">
  <form action="{{ route('dashboard.lessons.update', $lesson->id) }}"
    method="POST">
    @csrf
    @method('PUT')
    <div class="border-b p-3">
        ...
        <x-input name="category" type="select">
          ...
          @foreach (App\Models\Category::all() as $category)
            <option value="{{ $category->id }}"
              @if ($category->id === $lesson->category_id) selected @endif>
      ...
      <div class="flex items-center py-3">
        ...
        <label for="public-checkbox"
          class="ml-2 text-sm font-medium text-gray-900">Publikasikan
          pelajaran</label>
      </div>
    </div>
    <div class="p-3">
