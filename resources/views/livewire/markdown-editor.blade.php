<div x-data="markdown" class="border bg-white">
  <div class="flex">
    ...
    <button type="button"
      class="block items-center justify-center border-b-2 bg-white px-4 pb-2 pt-3 text-xs font-bold uppercase tracking-widest text-gray-500"
      :class="preview ? '!border-primary-600 !text-gray-700' : ''"
      @click="preview = true">Preview</button>
  ...
  <div :class="preview ? '!hidden' : ''">
    ...
    <label class="relative flex w-full pb-24">
      ...
      <textarea x-ref="textarea" wire:model="input" name="{{ $name }}"
        id="{{ $name }}"
        class="absolute bottom-0 left-0 right-0 top-0 z-10 block h-auto w-full resize-none overflow-visible !border-0 bg-transparent p-0 pl-10 text-sm leading-5 text-gray-700 !ring-0 selection:bg-primary-600 selection:text-white"></textarea>
    </label>
  </div>
