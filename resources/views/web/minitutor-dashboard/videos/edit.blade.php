...
@section('content')
<form class="ab-editor-layout" id="app-editor-layout" method="POST" action="{{ route('dashboard.minitutor.videos.update', $video->id) }}" enctype="multipart/form-data">
  @csrf
  @method('put')
  <header class="ab-header">
    <div class="ab-header__wrapper">
      <div class="ab-header__left">
        <a href="{{ route('dashboard.minitutor.videos.index') }}" class="ab-header__left-brand">
          <img src="{{ asset('img/logo/logo-white.svg') }}" alt="">
        </a>
      </div>
      <div class="ab-header__right">
        <div class="ab-header__right-actions">
          <a href="{{ route('dashboard.minitutor.videos.index') }}" class="btn btn-default">Batalkan</a>
        </div>
      </div>
    </div>
  </header>
        ...
        <div class="col-lg-9">
          <div class="panel panel-bordered">
            ...
            <div class="panel-body">
              ...
              <app-video-uploader upload-url="{{ route('dashboard.minitutor.videos.upload-video', $video->id) }}"></app-video-uploader>
            </div>
          </div>
        </div>
            ...
            <div class="panel-body">
              ...
              <div class="form-group">
                <div class="checkbox-custom checkbox-primary">
                  <input type="checkbox" id="publish-check" name="publish" @if($video->requested_at) checked @endif>
              ...
              <div class="form-group mt-15">
                ...
                <a href="{{ route('dashboard.minitutor.videos.index') }}" class="btn btn-default">Kembali</a>
              </div>
            </div>
          </div>
        </div>
...
@foreach($video->videos as $vid)
<form id="form-des-video-list-{{$vid->id}}" action="{{ route('dashboard.minitutor.videos.destroy-video', [$video->id, $vid->id]) }}" method="post" class="d-none">
  @csrf
  @method('delete')
</form>
@endforeach
