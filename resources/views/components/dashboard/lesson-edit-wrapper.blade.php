  ...
  <div class="mb-3 border bg-white">
    <div class="flex items-center border-b px-3 py-5">
      ...
      <form x-data="{
        ...
        @csrf
        @method('DELETE')
        <x-button type="button" @click="onclick" x-bind:disabled="loading"
          variant="red"
    ...
    <div class="flex flex-col border-b md:flex-row">
      <div class="relative p-3 md:w-52">
        @if ($lesson->public)
          <span
            class="bg-primary absolute left-2 top-2 rounded bg-primary-600 px-2 py-1 text-xs text-white shadow">PUBLIK</span>
        @endif
        <img class="block w-full rounded"
      ...
      <div class="flex-1 p-3 pt-0 md:pl-0 md:pt-3">
        <div class="mb-2">
          ...
          @if ($lesson->category)
            <span
              class="rounded-sm border bg-gray-100 px-2 py-1 text-2xs font-semibold">{{ $lesson->category->name }}</span>
          @endif
        </div>
