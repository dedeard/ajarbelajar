...
@php
  ...
  if ($variant === 'primary') {
      ...
      $btnClass = $btnClass . ' bg-red-600 hover:bg-red-700 hover:disabled:bg-red-600 border-red-600 hover:border-red-700 hover:disabled:border-red-600';
      $loadingFill = 'fill-red-600';
  }
@endphp
...
<button {{ $attributes->merge(['type' => 'submit', 'class' => $btnClass]) }}>
  ...
  <span aria-hidden="true"
    class="absolute left-1/2 top-1/2 hidden -translate-x-1/2 -translate-y-1/2 transform group-disabled:block">
    <svg class="{{ $loadingFill }} h-6 w-6 animate-spin text-white"
