...
<div class="flex flex-col border bg-white">
  <div class="block w-full flex-1 p-2">
    <div class="relative mb-2">
      @if ($lesson->category)
      <a href="{{ route('categories.show', $lesson->category->slug) }}" class="absolute left-1 top-1 block bg-primary-600 px-2 py-1 text-2xs font-semibold leading-none text-white shadow hover:shadow-lg">
      ...
      <div class="absolute -bottom-4 right-1 h-9 w-9 rounded-full bg-white p-px shadow-lg">
        <img src="{{ $lesson->user->avatar_url }}" alt="{{ $lesson->user->name }}" class="block h-full w-full rounded-full border border-white" />
      </div>
    </div>
  ...
  <div class="border-t">
    <a href="{{ route('lessons.show', $lesson->slug) }}" class="block bg-gray-50 py-2 text-center text-sm font-semibold uppercase tracking-wider hover:text-primary-600">
      Tonton
    </a>
  </div>
</div>
