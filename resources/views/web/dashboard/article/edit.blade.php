...
@section('content')
<form class="ab-editor-layout" id="app-editor-layout" method="POST" action="{{ route('dashboard.article.update', $article->id) }}" enctype="multipart/form-data">
  @csrf
  @method('put')
  <header class="ab-header">
    <div class="ab-header__wrapper">
      <div class="ab-header__left">
        <a href="{{ route('dashboard.article.index') }}" class="ab-header__left-brand">
          <img src="{{ asset('img/logo/logo-white.svg') }}" alt="">
        </a>
      </div>
      <div class="ab-header__right">
        <div class="ab-header__right-actions">
          <a href="{{ route('dashboard.article.index') }}" class="btn btn-default">Batalkan</a>
        </div>
      </div>
    </div>
  </header>
        ...
        <div class="col-lg-9">
          <div class="panel">
            ...
            <div class="panel-body">
              <app-editorjs image-url="{{ route('dashboard.article.image', $article->id) }}" editor-body="{{ $article->body }}"></app-editorjs>
            </div>
          </div>
        </div>
        <div class="col-lg-3">
