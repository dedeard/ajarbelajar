<x-app-layout dashboard title="Aktifitas saya" header="Aktifitas saya">
  ...
  <div class="container p-3">
    <div class="grid-col-1 grid gap-3">
      ...
      <a href="{{ route('lessons.watch', ['lesson' => $activity->episode->lesson->slug, 'index' => $activity->episode->index]) }}" class="flex border bg-white p-3 hover:bg-gray-50">
        <div class="w-16 md:w-24">
          <img class="block w-full" src="{{ $activity->episode->lesson->cover_url['small'] }}" alt="{{ $activity->episode->lesson->title }}" />
        </div>
