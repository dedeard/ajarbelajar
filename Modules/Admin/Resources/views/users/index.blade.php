    ...
    <div class="card">
      <div class="card-header d-flex">
        ...
        <div class="my-auto ml-auto">
          @can('create user')
            <a href="{{ route('admin.users.create') }}" class="btn btn-sm btn-primary">Buat Pengguna Baru</a>
          @endcan
        </div>
      </div>
        ...
        <table class="table">
          ...
          <tbody>
            @foreach ($users as $user)
              <tr>
                <td class="py-0 align-middle"><img src="{{ $user->avatar_url }}" width="36px" alt="Avatar" class="avatar img-circle">
                </td>
                <td>{{ $user->name }}</td>
                <td>{{ $user->username }}</td>
                <td>{{ $user->email }}</td>
                <td class="py-0 text-center align-middle">
                  <div class="btn-group">
                    @can('update user')
                      <a href="{{ route('admin.users.edit', $user->id) }}" class="btn btn-default btn-sm" title="Edit pengguna">
                    ...
                    @can('delete user')
                      ...
                      <button class="btn btn-danger btn-sm btn-icon" title="Hapus pengguna"
