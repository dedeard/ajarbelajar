<li class="flex px-3 py-5 odd:bg-gray-50">
  ...
  <div class="flex-1">
    <div class="mb-3 flex h-10 flex-col justify-center">
      <a href="{{ route('users.show', $comment->user->username) }}"
        class="block text-sm font-semibold capitalize text-primary-600">
        {{ $comment->user->name }}
      </a>
      <span
        class="block text-xs">{{ $comment->created_at->diffFOrHumans() }}</span>
    </div>
    ...
    <div class="flex justify-end">
      ...
      @endif
      <button
        class="@if ($comment->liked()) text-red-600 @endif flex items-center justify-center leading-none"
        type="button" wire:click="likeToggle" @guest disabled @endguest>
        <span class="block">
