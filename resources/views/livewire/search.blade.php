  ...
  @if ($open)
    <div class="fixed inset-0 z-50 bg-black bg-opacity-50">
      <div class="relative flex justify-center lg:container lg:py-14">
        <div x-on:click.stop.outside="() => window.Livewire.emit('closeSearchModal')"
          ...
          <div class="min-h-16 relative flex w-full bg-white">
            ...
            <input wire:model="input"
              class="block h-16 w-full flex-1 bg-transparent pl-12 pr-3 text-xl font-semibold leading-none text-gray-500 placeholder-gray-300 outline-none transition-colors focus:text-gray-700"
              placeholder="Apa yang anda cari?" @focus="focus = true" @blur="focus = false" />
            @if ($input)
              <button
            ...
            <button
              class="my-auto flex h-full cursor-pointer items-center justify-center px-4 text-sm font-semibold leading-none text-red-600 transition-colors hover:bg-gray-50"
              @click="() => window.Livewire.emit('closeSearchModal')">Close</button>
          </div>
          @if (count($results))
            ...
            <div class="custom-chrome-scrollbar flex max-h-full flex-col overflow-y-auto bg-white">
              @foreach ($results as $result)
                <a href="{{ route('lessons.show', $result['slug']) }}" class="group block cursor-pointer bg-gray-100 px-4 hover:bg-white">
                  <div class="last:border-b-5 flex border-b py-4">
                    <div class="flex items-center">
                      <div class="block">
                        <img src="{{ $result['cover_url'] }}" alt="{{ $result['title'] }}" class="h-7 rounded" />
                      </div>
                    </div>
          ...
          @if (count($results) === 0 && strlen($queryResult) > 1 && strlen($input) > 1)
            <div class="flex max-h-full flex-col overflow-y-auto bg-gray-100 first:block">
              <div class="py-16 text-center">
                ...
                <h3 class="text-2xl font-semibold text-primary-600">{{ $queryResult }}</h3>
              </div>
            </div>
          @endif
        </div>
